...
def build_cover_prompt(
    ...
    parts = [
        ...
        f"封面主标题：{cover_title}。",
        (
            "主标题必须是画面第一视觉中心，采用超大字号、粗重字体和高对比配色。"
            "标题文字块应占据画面高度约45%至60%，宽度尽量占满安全区域。"
        ),
        (
            "主标题采用2到3行大字排版，单行汉字尽量占满画面宽度。"
            "每个汉字笔画粗重、字面饱满，最小字高不低于画布高度的8%。"
            "如果主标题超过10个汉字，优先拆行而不是缩小字号。"
        ),
    ]
    if cover_subtitle:
        parts.append(
            f"封面副标题：{cover_subtitle}。副标题字号必须明显小于主标题，只能作为辅助信息。"
        )
    if theme:
    ...
    parts.extend(
        [
            f"视觉风格：{style_prompt}",
            (
                "封面布局：标题集中在画面上半区，使用手绘杂志封面和社交平台爆款封面式大字排版。"
                "重点词可以使用不同颜色、粗描边、贴纸底或色块承托，形成强烈的文字海报感。"
                "图标、箭头、便签和装饰元素只能放在标题周围，不能抢走标题视觉重量。"
            ),
            "信息密度适中，重点突出，留白充足，手机小屏缩略图下仍然清晰可读。",
            "标题文字不出现错别字、乱码、缺字、重复字或笔画粘连。",
